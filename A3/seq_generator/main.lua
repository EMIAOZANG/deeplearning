...
function write_result(run_data, epoch, metric)
   ...
   file:write(params.architecture..':\t'..run_data..'\t'..epoch..'\t'..metric)
   file:close()
end
...
function run_valid()
    ...
    write_result("validation", epoch, g_f3(amortized_perp)) -- writing result to file
...
function run_test()
    ...
    write_result("test", epoch, g_f3(amortized_perp)) -- writing result to file
    g_enable_dropout(model.rnns)
end
...
-- save model to file
model_path = './models/'..(params.architecture)..'_final.net'
