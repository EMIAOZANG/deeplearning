...
cmd = torch.CmdLine()
cmd:text()
cmd:text('Prediction Generatation Using Trained Models')
cmd:text()
cmd:text('Options:')
cmd:option('-modelPath','results/','subdirectory of model.net file')
cmd:option('-size','full','size of data small | full')
cmd:option('-visualize',false, 'enable visualizations')
cmd:text()
...
-- RUN
predictions = predict(paths.concat(opt.modelPath,"model.net"))
save_pred_file(paths.concat(opt.modelPath,"predictions.csv"),predictions)
